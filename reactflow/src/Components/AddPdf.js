...
function TextUpdaterNode({ data, onRemove, id }) {
  ...
  return (
    <>
      <button onClick={onRemove} className="text-gray-600 hover:text-red-800 text-3xl mb-4">
        <MdCancel />
      </button>
    ...
    <div className='bg-[#ffffff] rounded-[45px] p-7 shadow-2xl hover:border-solid border-[4px] border-green-600' onMouseOver={Show} onMouseOut={Leave}>
      ...
      <div className='bg-[#eae6df] border-[3px] border-solid border-red-600 rounded-[25px] p-3 mt-5'>
        ...
        <div className='block bg-[#eae6df] p-2 rounded-xl w-full'>
          <textarea id="w3review" name="w3review" rows="4" cols="35" placeholder='Enter the caption' className='rounded-2xl border-green-600 border-3 text-3xl' style={{ border: "none" }} value={message} onChange={handleMessageChange}></textarea>
        </div>
